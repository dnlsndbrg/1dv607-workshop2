...
.post(function doRegister(req, res) {
    ...
    .then(member => res.redirect(`/members/${member.id}`))
    .catch(e => res.render("register-member"));
});
...
.get(function displayCompactList(req, res) {
    ...
    .then(members => res.render("list-members-compact", { members: members }));
});
...
.get(function displayVerboseList(req, res) {
    memberRegistry.getAll()
    .then(members => res.render("list-members-verbose", { members: members }));
});
...
.post(function doEdit(req, res) {
    ...
    .then((member) => {
        ...
        .then(member => res.redirect(`/members/${member.id}`))
        .catch(e => res.render("register-member", {member}));
    });
});
...
.get(function doDelete(req, res) {
    ...
    .then(member => {
        ...
        member.delete()
        .then(id => res.redirect("/members/list/compact"))
